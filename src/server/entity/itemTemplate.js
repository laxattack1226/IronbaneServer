...
// todo: more generic query? or just keep making these methods?
ItemTemplate.getAllByType = function(type) {
    var deferred = Q.defer();

    // check cache
    if (_.keys(cache.cache).length === 0) {
        ItemTemplate.getAll().then(function(templates) {
            var results = _.where(templates, {type: type});
            deferred.resolve(results);
        });
    } else {
        var templates = _.where(cache.cache, {type: type});
        deferred.resolve(templates);
    }
