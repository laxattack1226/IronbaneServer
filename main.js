...
var LoadActorScripts = function() {
    ...
    return fs.listTree(scriptPath, function(file) {
        ...
        _.each(files, function(file) {
            ...
            try {
                var script = require(path.relative('./', file));
...
function start(scripts) {
    ...
    // temp global references to these entity-services until all can be required
    global.ItemTemplate = require('./src/server/entity/itemTemplate');
    global.Item = require('./src/server/entity/item');
    global.UnitTemplate = require('./src/server/entity/unitTemplate');

    // hacky! - pre-load the loot bag template (because of async madness!)
    global.UnitTemplate.get(IB.lootBagTemplate).then(function(template) {
        global.lootBagTemplate = template;
    }, function(err) {
        console.error('Error loading global lootBagTemplate!', err);
    });
    ...
    // temp global Q for async in the legacy code
    global.Q = require('q');
    ...
    global.chatHandler = require('./src/server/game/chat')(io, global.worldHandler);
